...
| X | 直接发送当前 composer 文本 | 已实现；优先调用 Codex 当前 Send/Submit 按钮，找不到时使用 `composer.submit` 的配置键（默认 F22）并验证输入框清空；绝不降级为 Enter |
...
| R3 单击 | 打开与当前简易 / 高级设置对应的原生模型菜单；菜单已打开时再次单击关闭 | 已实现；B 也可关闭菜单 |
