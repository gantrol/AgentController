...
| View | 切换当前受控 Agent | 已实现；在 Codex 与 DeepSeek Harness 间循环，即使当前目标离线也可切换；组合层内仍保留原有 View 槽位语义 |
| Menu | 需要时唤醒并置前当前 Agent | 已实现；目标已在前台时，连接手柄回中后自动启用，不必再按 Menu；桥接关闭时仍被总门控拦截 |
| Guide | 无动作 | 保留给系统 |
