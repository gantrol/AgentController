...
| R3 单击 | 简易模式打开官方模型列表；高级模式打开完整设置菜单；菜单已打开时再次单击关闭 | 已实现；简易入口调用 `composer.openModelPicker`，B 也可关闭；快捷键冲突或写入失败时不会误进入菜单状态 |
| A（选择器打开时） | 交给 Codex 原生 Enter：可能切换 Compact/Advanced、进入 Model/Effort/Speed 子菜单或确认叶子；模型会话继续保持 | 已实现；必须由 B/R3 显式结束，期间禁止回落到 F17/F18 Power 路径 |
...
| 简易 | R3 | 通过 `composer.openModelPicker` 打开官方层级选择器；方向导航，A 切换视图/进入子菜单/确认叶子，B/R3 结束会话；不枚举 UIA 列表 |
| 高级 | 右摇杆左/右 | 在 Model、Effort、Speed 三个模型相关控件间循环 |
| 高级 | 右摇杆上/下 | 展开当前控件的现场子菜单，严格按界面从上到下的视觉顺序移动（低档在最上；上拨选择上一项/更低档，下拨选择下一项/更高档）；界面回读确认后才报告成功 |
| 高级 | R3 | 打开并同步 Codex 高级模型菜单，不改当前选择；再次单击关闭 |
...
- 原生输入 ACK 不携带菜单层级或关闭状态；A/Enter 后继续锁定模型会话，避免
  Codex 仍处于子菜单时把右摇杆误路由为 F17/F18。选择完成后用 B/R3 显式退出。
`models_cache.json` 只用于选中后的状态显示，绝不用于生成或补齐可切换档位；

- R3 会先要求 `ModelPickerShortcut` 已成功写入且没有按键冲突。首次新增快捷键后
  Codex 若未热加载，需要重启一次。键盘注入成功不是命令动作回执；在“不枚举 UIA
  列表”的边界下，Codex 丢弃快捷键时只能由 B/R3 清理本地状态并进行真机复测；
