# Agent Controller v0.7.0-hotfix

发布日期：2026-07-16

Hotfix：2026-07-17（LB 状态、X 提交、B 长按、简易模型选择、置顶项目兼容与管道降噪）
...
- 简易模式直接控制实时 Power、Standard 与 Fast；短按 R3 通过官方
  ...
  会在输入前拦截，不会误进入模型菜单状态。
- 后续 Micro-first 修订统一了模型选择器：上/左为上一项，下/右为下一项，R3 打开/进入/确认，B 通过官方 `AG00` 上下文返回退出；不再由左右承担进入/退出。
...
- transport Accepted 不是界面动作回执；选择叶子后仍需 B 显式结束本地 Micro 菜单会话，仍需
  当前版本真机冒烟。
- 部分功能在首次写入或更新 Codex 快捷键后，需要重启 Codex 桌面软件
  （ ChatGPT）才能生效。
