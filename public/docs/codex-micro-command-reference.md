...
- 唯一的 `AgentController.MicroBroker` 使用 `GET_INFO`、`SUBMIT_INPUT`、`READ_OUTPUT`，合同仍保留遗留 `SUBMIT_KEYBOARD`；
- `codex-micro-v0.1.0` 的同一 UMDF2 source 提供受限键盘 child，只允许 Tab、Shift+Tab、Enter；当前 Agent Controller 与虚拟 Micro 均不调用它。旋转仍发送原生 `ENC_*`；“仅推理强度”模式的短按模型切换改走当前任务 owner 的语义设置通道。
