...
flowchart TD
    ...
    Window --> Config["配置页"]
    Window --> Settings["设置页"]
    Window --> Footer["底栏：最近状态 / Full reset 过期时间 / 版本"]
...
| 底栏 | 高 `38`；左侧事件/状态，中间显示可用 Full reset 的本地精确过期时间，右侧版本 |
...
侧边栏弹层把“选择 scope/项目”和“选择任务”分成相邻卡片：左摇杆上下只移动 Agent Controller 的本地光标，左右进入/退出层级，A 打开任务，L3 循环根 scope。分类按钮、目录选择与任务打开必须分离；前两者不得调用 Codex UIA `SetFocus`，避免新版 Codex 把焦点动作解释为任务跳转。
...
| 底栏 | 最近一次非阻塞状态与可用 Full reset 的本地精确过期时间 | 不遮挡、不抢焦点；额度数据读取失败或没有可用 credit 时隐藏 |
