...
public sealed class CodexForkThreadActionExecutor : CodexActionExecutorBase
{
    ...
    {
        if (request.ActionId != ForkThreadActionContract.Id)
        {
        ...
        if (_tryMicro is null &&
            _tryShortcut is null &&
            _invokeAutomation is null)
        {
            return new ExecutorCapability(
                Id,
                request.ActionId,
                ExecutorCapabilityStatus.Unsupported,
                Priority: 100,
                ReasonCode: "agent.thread-fork.unavailable");
        }
