...
- [x] LB Agent 使用专用六灯面板；RB Command、RT Turn、Y Action 复用同一套菜单卡片、行状态和按键提示。键帽只显示实际 glyph，ABXY 物理位置使用明确的菱形图例，不再叠加会被误读为十字键组合的裸方向箭头；转盘不再作为通用容器。
- [x] 首页手柄主卡提供动态教程，可点击或用真实手柄切换基础、点按 Y、按住 LB、扣住 RT、按住 RB 和摇杆按压六种状态；教程与 Overlay 共用控制层 presentation，并使用当前设备 profile 的 View/Menu glyph。L3/R3 同时展示 LS/RS 别名、摇杆帽垂直按压图解和按压动画，明确区别于向下拨动摇杆。
