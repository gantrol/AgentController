...
- [x] IPC 已包含 protocol version、driver epoch、request id、长度上限、当前用户 pipe、单客户端心跳 lease 和有界并发；全局 batch sequence 只由 Broker 驱动端点分配。
- [ ] 为超时后由调用方显式重试的非幂等 request 增加 response cache / 幂等去重；当前客户端不会自动重试输入，但 Broker 尚未缓存重复 request id。
...
- [x] 不用共享句柄冒充多客户端支持；held key、PTT 和 analog 按客户端持有 lease，客户端断开时只中和自己的状态，同键/同 analog 由另一客户端持有时不会误释放。
