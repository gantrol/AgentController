...
- [x] 默认映射 LT→ACT10 down/up、X→已验证 Codex slot、右摇杆上/左→`ENC_CW`、下/右→`ENC_CC`、R3→press/hold、RB+右摇杆→Analog；不再保留横向 UIA Left/Right 路径。
...
- [ ] 依据实物照片评估并重做模拟器外观；至少记录 Agent 键帽为磨砂透光、整键 RGB 扩散，而不是固定紫色圆点。外壳、螺丝、旋钮、轴体、面板文字和背面等具体还原范围与优先级待定。
