...
> Status: Maintenance
> Priority: Bug fixes and compatibility only
> New architecture work belongs in the numbered files in this directory.
...
- v0.7 shipped behavior / remaining validation
  - Consolidated controller-input symptoms, reproduction rules, and physical acceptance are tracked in [`91-controller-input-known-issues.md`](91-controller-input-known-issues.md); do not close the scattered model-controller or LT items below without updating that ledger.
  ...
  - [ ] Investigate the existing intermittent model-controller input loss: model controls sometimes stop responding to the gamepad, while opening the model picker makes control work again; the picker is a reproduction clue, not an accepted workaround, and this issue does not block the architecture migration
    - Record the exact sequence and whether the picker was opened by R3, mouse, keyboard, or a Power/Fast action; cover Simple/Advanced mode, foreground/focus changes, picker close/reopen, and held-versus-neutral stick state
    - Capture `_virtualDialMenuOpen`, `_simpleModelPickerOpen`, `_composerPickerMenuLikelyOpen`, active input context, foreground/automation gates, pending/coalesced dial work, cancellation/session ownership, and controller snapshots immediately before and after recovery
    - Determine whether stale picker ownership, an uninitialized picker session, input draining/neutralization, focus gating, or model-catalog/readback state is swallowing the route; do not infer the cause solely from the recovery symptom
    - Add a deterministic regression test for the confirmed state transition and repeat physical validation across the supported controllers before closing the issue
  - [ ] Investigate the existing intermittent LT push-to-talk failure as a deferred runtime-maintenance issue; it is tracked independently and does not block the architecture migration
    - Record whether the LT edge is absent, blocked, starts without stopping, or reaches Codex without usable dictation; capture raw `LeftTrigger`, `AnalogTriggerLatch` transitions, `PushToTalkInputPolicy` block reason, foreground/dial context, automation queue/cancellation, and Codex dictation readback
    - Reproduce with model picker and radial contexts open/closed, foreground changes, neutral/re-engage cycles, and each supported controller
    - Identify the failing layer before assigning the issue to controller transport, input policy, automation, focus continuity, or Codex readback
    - Do not change trigger thresholds or automation routing until the failing layer is identified and covered by a deterministic regression test
  - [ ] Stabilize the intermittent `BridgeFeedbackPresenterTests.DisposeUnsubscribesAndQueuedWorkCannotMutateState` queue/dispose race; six full-suite runs observed a queued row after disposal, while isolated reruns pass; the latest 688-test baseline requires a documented 687-test solution run excluding this case plus a passing isolated run
    - Make the test control synchronization deterministically instead of depending on dispatcher scheduling; verify both unsubscribe and already-queued callback behavior
    - Keep this separate from the action-routing migration unless a deterministic reproduction proves a shared cause
...
- v0.7 deferred / experimental work
  ...
  - [x] Transfer Micro/VHF protocol evidence and future implementation out of v0.7 maintenance into `todo/03-codex-micro-compatibility.md` and ADR-0002
  - [x] Keep v0.7 free of a shipped/installed virtual HID driver; future Full Micro mode uses the separately signed Device Support lifecycle defined by ADR-0002
...
- v0.4a -> v0.4b controller mapping preparation
  ...
  - v0.4b implementation order
    ...
    - [ ] Apply long-hold, second confirmation, or a dedicated confirmation page to every R2 route
      - [x] Current Command-panel Approve uses a same-action second press with a 2.5 second expiry; release, panel reset, or an intervening action cancels it
      - [ ] Require verified `ApprovalRequest` context before exposing Approve through configurable bindings
