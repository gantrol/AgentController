...
- v0.7 shipped behavior / remaining validation
  ...
  - [ ] Stabilize the intermittent `BridgeFeedbackPresenterTests.DisposeUnsubscribesAndQueuedWorkCannotMutateState` queue/dispose race; one full-suite run observed a queued row after disposal, while 10 immediate isolated reruns and the following 644-test full run passed
