...
- 通知区域图标双击可显示/收起；右键菜单提供“显示/收起小键盘”、“重启 Codex Micro”
  和“退出”。重启会先释放录音、按住的 Micro 键、摇杆状态和 Broker lease，接班进程
  会等待旧的单实例进程完全退出后再启动；
...
第三颗状态灯只有在语音提供商与 Windows 录音设备都可用时才显示蓝色；若 Qwen
服务健康但没有已连接或已启用的麦克风，则显示错误灯；按下语音键时会持续显示
“连接 / 启用麦克风后重试”的操作提示。语音进度和恢复提示统一显示在机身底部的
大状态区与设置键中央，不再使用语音键下方的小字徽标。若小键盘持有的本地 Qwen 流式链路中断，
小键盘会自动重启服务并重试一次；外部管理的服务不会被误杀，而会提示手动重启。
Bridge 只在 DeepSeek 与小键盘之间转发“切换语音”请求以及增量 / 最终文字；DeepSeek 插件只
增加一个语音按钮，不采集音频、不连接 Qwen，也没有语音设置页。

流式听写期间一旦用户手动编辑输入框，Bridge 会切入手改保护模式：后续只追加识别出的
新增尾段，识别器对既有片段的修订以及取消操作都不会覆盖或回滚用户修改。

DeepSeek 语音开始前还会确认其专用窗口位于前台：先恢复并置前已有窗口；若窗口尚未
启动，则先执行正常的 Harness 启动链路，再打开麦克风。
...
空白的 Codex 新会话草稿尚没有 App Server thread。2026-09-02 的 E2E 已确认，当前
config + encoder seed 实现只能改变 renderer 现有模型的思考强度，不能可靠切换模型。
接受的修复仅在空白草稿阶段操作前台官方 Composer：按语义选择精确模型，再通过独立
`Power` 控件选择强度；根据公开档位总数计算绝对目标。有可写 RangeValue 时一次设置；
当前构建没有时，从带 `N of M` 状态的唯一滑轨节点取得实时矩形，只点击目标刻度一次并
恢复鼠标位置。不再先向左再向右扫描；缺少安全直接动作时失败关闭。实现不使用
固定坐标、列表序号或盲发步数。事务期间额度环显示
加载动画并拒绝重复短按。若出现 Ultra + Full access 警告，等待用户在 Codex 中决定，
不自动确认权限；弹窗关闭后重新读取最终状态。操作开始前已存在的警告不会被新事务接管。
DEBUG E2E 不输入或提交探针文本，不创建、导航、激活 Codex，也不循环抢回前台；它只在
用户已经打开并置前的空白草稿中运行，失焦立即停止。第一轮创建真实任务后租约立即失效，后续
切换恢复为当前任务的语义设置通道。实现状态与版本边界见
...
0.3.0 只更新 Monitor 应用，不替换也不捆绑虚拟 HID 驱动。已有驱动安装结果为
`Ready` 时无需重装；尚未安装或未达到 `Ready` 时，仍需按上面的指南单独安装。
...
- `dist/Codex-Micro-Monitor-v0.3.0-win-x64.zip`，不含 DeepSeek Bridge 或托管
  WSL 安装流程的通用 Codex 包；

- `dist/Deepseek-Harness-Keypad-v0.3.0-win-x64.zip`，包含 Bridge 与自动配置入口；
  首次配置时在线安装固定的 DSH，不内置完整 WSL 根文件系统。

以下是维护者构建与诊断产物，不作为普通用户的下载选项：

- 单文件可执行程序：`.artifacts/micro-release/0.3.0/publish/CodexMicro.exe`；
- 独立压缩包：`dist/CodexMicro-Keypad-0.3.0-win-x64.zip`；
- SHA-256：同名 `.sha256` 文件。
- 可单独安装到已有 DSH 的 Bridge：
  `dist/Deepseek-Harness-Keypad-Bridge-v0.3.0.zip`（也带独立 SHA-256）。
