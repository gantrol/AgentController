...
public sealed class KeycapIcon : FrameworkElement
{
    ...
    // Exact fish mark used by the local DeepSeek Harness. Its native
    // 23.16 x 17.04 viewBox is fitted into the keycap's 20 x 20 canvas.
    private static readonly IReadOnlyList<Geometry> DeepSeekGeometry =
        CreateGeometry(
    ...
    protected override void OnRender(DrawingContext drawingContext)
    {
        ...
        switch (KeycapId)
        {
            ...
            case "CODEX":
                ...
                break;
            case "DEEPSEEK":
                DrawFittedGeometry(drawingContext, brush, DeepSeekGeometry);
                break;
            case "HARNESS":
            ...
            case "TIME":
                ...
                break;
            case "GOAL":
                DrawGoal(drawingContext, pen, brush);
                break;
            case "MIND+":
    ...
    private static void DrawFittedGeometry(
        DrawingContext context,
        Brush brush,
        IReadOnlyList<Geometry> geometry)
    {
        var bounds = geometry.Aggregate(Rect.Empty, (current, item) =>
            current.IsEmpty ? item.Bounds : Rect.Union(current, item.Bounds));
        var scale = Math.Min(18.4 / bounds.Width, 18.4 / bounds.Height);
        context.PushTransform(new TranslateTransform(10, 10));
    ...
    private static void DrawGoal(DrawingContext context, Pen pen, Brush brush)
    {
        context.DrawEllipse(null, pen, new Point(9.2, 10.8), 6.6, 6.6);
        context.DrawEllipse(null, pen, new Point(9.2, 10.8), 3.2, 3.2);
        context.DrawEllipse(brush, null, new Point(9.2, 10.8), 0.9, 0.9);
        context.DrawGeometry(
            null,
            pen,
            Geometry.Parse("M10 10 L17.2 2.8 M13.6 2.8 H17.2 V6.4"));
    }
