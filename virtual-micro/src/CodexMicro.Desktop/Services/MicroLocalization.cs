...
internal static class MicroEnglishTranslations
{
    ...
    [
        ...
        ("左键：通过原生 Micro 输入打开官方设置 · 右键：直达右下角当前 Agent 的软件设置。", "Left-click: open official Micro settings through the native Micro input path · Right-click: open the current Agent's software settings."),
        ("圆环显示当前更紧张的额度窗口。左键通过原生 Micro 输入打开官方设置；右键直达右下角当前 Agent 的软件设置。", "The ring shows the tighter window. Left-click opens official Micro settings through the native Micro input path; right-click opens the current Agent's software settings."),
        ("正在等待网页桥接回报当前模型。", "Waiting for the browser bridge to report the current model."),
        ("当前模型：{currentModel}。", "Current model: {currentModel}."),
        ("左键切换到 DeepSeek Harness 模型目录中的下一个模型；右键直达此 Agent 的适配器与独立键位设置。", "Left-click switches to the next model in the DeepSeek Harness catalog. Right-click opens this Agent's adapter and key-map settings."),
        ...
        ("DeepSeek 托管版本检查失败：{exception.Message}", "Managed DeepSeek version check failed: {exception.Message}"),
        ...
        ("rc.8 的存储格式不兼容。升级会完整保留旧运行时与旧会话作为备份，只把凭据和设置复制到全新的 rc.8 数据目录，不会把旧会话导入 rc.8。\n\n现在升级吗？", "rc.8 uses an incompatible storage format. The old runtime and conversations will be kept as a backup, while credentials and settings are copied into a fresh rc.8 home. They will not be imported into rc.8.\n\nUpgrade now?"),
        ("托管 DeepSeek 继续使用 {current}；未修改任何文件。", "Managed DeepSeek remains on {current}; no files were changed."),
        ("正在继续上次未提交的 rc.8 健康检查。", "Resuming the pending rc.8 health check."),
        ("正在准备可回滚的托管升级。", "Preparing a rollback-safe managed upgrade."),
        ("已取消托管 DeepSeek 升级；旧版备份仍然保留。", "The managed DeepSeek upgrade was cancelled; its backup was retained."),
        ("DeepSeek 托管升级失败：{exception.Message}", "Managed DeepSeek upgrade failed: {exception.Message}"),
        ("{result.Step}/8 升级需要处理", "Upgrade needs attention at {result.Step}/8"),
        ("升级托管 DeepSeek Harness", "Upgrade managed DeepSeek Harness"),
        ("升级检查失败", "Upgrade check failed"),
        ("升级需要处理", "Upgrade needs attention"),
        ("备份旧运行时", "Back up runtime"),
        ("8/8 升级完成", "8/8 upgraded"),
        ("缺失", "missing"),
        ...
        ("语音采集与识别均在此小键盘配置；语音通路只向 DeepSeek 发送最终文字。图片输入由 Harness 单独处理，并取决于当前模型是否支持。", "Voice capture and recognition are configured on this keypad; the voice path sends final text to DeepSeek. Image input is handled separately by Harness when the selected model supports it."),
        ...
        ("未检测到可用的 Windows 麦克风。请连接或启用录音设备后重试。", "No active Windows microphone is available. Connect or enable a recording device, then try voice input again."),
        ("未检测到可用的 Windows 麦克风。请打开“设置 → 系统 → 声音 → 输入”，连接或启用麦克风后再次点按语音键。", "No active Windows microphone is available. Open Settings > System > Sound > Input, connect or enable a microphone, then press the voice key again."),
        ("小键盘已自动重启本地 Qwen 语音服务一次，但链路仍未恢复。请手动重启 Qwen 语音服务后再次点按语音键。\n{result.Message}", "The keypad restarted the local Qwen voice service once, but the link is still broken. Restart the Qwen voice service manually, then press the voice key again.\n{result.Message}"),
        ("小键盘未能自动恢复 Qwen 语音服务。请手动重启 Qwen 语音服务后再次点按语音键。\n{result.Message}", "The keypad could not recover the Qwen voice service automatically. Restart it manually, then press the voice key again.\n{result.Message}"),
        ("语音输入未能启动。请打开小键盘语音设置，执行“保存并验证”后重试。\n{result.Message}", "Voice input could not start. Open keypad voice settings, run Save and verify, then try again.\n{result.Message}"),
        ("未能调起并置前 DeepSeek。请从任务栏打开 DeepSeek，然后再次点按语音键。", "DeepSeek could not be opened in the foreground. Open DeepSeek from the taskbar, then press the voice key again."),
        ("DeepSeek 当前不在前台；小键盘正在先调起并置前 DeepSeek，再开始语音采集。", "DeepSeek is not in the foreground. The keypad is opening and focusing it before voice capture."),
        ("调起 DeepSeek 失败：{exception.Message}", "DeepSeek activation failed: {exception.Message}"),
        ("本地语音链路失败；小键盘正在重启其持有的 Qwen 服务，并自动重试一次。", "The local voice link failed. The keypad is restarting its Qwen service once and will retry automatically."),
        ("打开 DeepSeek 后重试", "Open DeepSeek, then retry"),
        ("连接 / 启用麦克风后重试", "Enable a microphone, then retry"),
        ("自动重启未恢复 · 请手动重启", "Automatic restart did not recover · restart manually"),
        ("重启语音服务后重试", "Restart the voice service, then retry"),
        ("检查语音设置后重试", "Check voice settings, then retry"),
        ("正在调起 DeepSeek", "Opening DeepSeek"),
        ("正在重启语音服务", "Restarting voice service"),
        ...
        ("当前 Codex 页面不是任务；请先打开一个本地任务，模型保持不变。", "The current Codex page is not a task. Open a local task first; the model was not changed."),
        ("检测到多个 Codex 任务窗口，无法唯一确定目标；模型保持不变。", "Multiple Codex task windows are visible, so the target is ambiguous; the model was not changed."),
        ("当前任务的语义状态暂不可用；模型保持不变，请稍后重试。", "The current task's semantic state is temporarily unavailable; the model was not changed. Try again shortly."),
        ("Codex 未确认模型设置；模型保持不变。", "Codex did not confirm the model setting; the model was not changed."),
        ("Codex 任务设置通道超时，未收到结果确认；请以当前任务显示的模型为准。", "The Codex task-settings channel timed out without a confirmed result; use the model currently shown for this task as the source of truth."),
        ("切换过程中当前 Codex 任务发生变化；已停止提交模型设置。", "The current Codex task changed during the switch, so the model update was stopped."),
        ("当前 Codex 版本的任务设置通道不可用；模型保持不变。", "The task-settings channel is unavailable in this Codex build; the model was not changed."),
        ("默认强度", "default effort"),
        ...
        ("打开详情栏", "Open details"),
        ("关闭详情栏", "Close details"),
        ("没有找到 Codex 主窗口，也无法启动已安装的 Codex 应用。", "No Codex main window was found, and the installed Codex app could not be started."),
        ("已请求启动 Codex，但在等待时间内没有找到其主窗口。", "Codex was asked to start, but its main window did not appear before the timeout."),
        ("当前没有可激活的 Codex 主窗口。", "No Codex main window is currently available to activate."),
        ...
        ("快捷模型切换超时，未收到 Codex 确认；请以当前任务显示的模型为准后再试。", "The quick-model switch timed out without Codex confirmation; check the model currently shown for this task before trying again."),
        ...
        ("Codex 键在应用不位于前台时只启动或置前；置前后再次按下才发送当前输入。", "When Codex is not in the foreground, the Codex key only starts or focuses it; press again after it is in front to send the current input."),
        ...
        ("隐藏面板", "Hide panel"),
        ("确认权限", "Confirm access"),
        ("确认框暂不支持旋钮 · 请在 Codex 操作", "Dial input is not supported in this dialog yet · Use Codex"),
        ("权限模式", "Access mode"),
