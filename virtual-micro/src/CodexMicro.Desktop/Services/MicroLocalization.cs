...
internal static class MicroEnglishTranslations
{
    ...
    [
        ...
        ("当前任务模型仍在同步；请稍后再试。", "The current task model is still syncing. Try again shortly."),
        ("当前页面尚未确认为空白新会话；模型保持不变，请稍后再试。", "The current page has not been confirmed as a blank new task. The model was not changed; try again shortly."),
        ("这个空白新会话的前台凭据已过期；为避免切到后台旧任务，模型保持不变。请重新打开空白新会话后再切换。", "The foreground proof for this blank new task is too old. To avoid changing a background task, the model was not changed. Open a fresh blank task before switching."),
        ("新会话凭据未续签", "New-task proof was not renewed"),
        ("新会话模型结果未知", "New-task model outcome unknown"),
        ("目标配置已写入，但重建后的新会话凭据未能续签；", "The target config was written, but the rebuilt new-task proof could not be renewed. "),
        ("目标已确认，但前台新会话凭据未能续签；", "The target was confirmed, but the foreground new-task proof could not be renewed. "),
        ("为避免下次命中后台任务，请重新打开空白新会话后再切换。", "To avoid targeting a background task next time, open a blank new task before switching again."),
        ("已请求按 {name} · {effort} 重建新会话；", "Requested a rebuilt new task with {name} · {effort}; "),
        ("目标配置已确认，Full access 未更改。", "the exact config was confirmed and Full access was unchanged."),
        ("已请求切换到 {name}", "Requested switch to {name}"),
        ("新会话已用 {name} · {effort} 创建；", "New task created with {name} · {effort}; "),
        ("新会话已切换到 {name} · {effort}；", "New task opened with {name} · {effort}; "),
        ("新会话已刷新为 {name} · {effort}；", "New task refreshed to {name} · {effort}; "),
        ("新会话：{name} · {effort}；renderer 已确认。", "New task: {name} · {effort}; renderer confirmed."),
        ("前台 renderer 已原生确认 Sol，Ultra 通过配置回放应用，", "The foreground renderer accepted Sol natively and replayed Ultra without changing "),
        ("Full access 未更改。", "Full access."),
        ("Codex 默认值也已更新，后续新会话会沿用。", "Codex's default was updated for later new tasks."),
        ("当前新会话未能完成原生模型初始化；请将 Codex 保持在前台后重试。", "The current new task could not complete native model initialization. Keep Codex in the foreground and try again."),
        ...
        ("Codex 任务设置通道刚刚断开；模型未确认，请稍后再试。", "The Codex task-settings channel just disconnected. The model was not confirmed; try again shortly."),
        ...
        ("未能读取 Codex 的当前默认模型；配置未更改，请稍后重试。", "Could not read Codex's current default model. The configuration was not changed; try again shortly."),
        ("Codex 未确认默认模型写入；已停止新建会话，请稍后重试。", "Codex did not confirm the default model update. No new task was opened; try again shortly."),
        ("当前项目配置覆盖了默认模型；为避免显示与实际不一致，本次未切换。", "The current project overrides the default model. The switch was cancelled to keep the display consistent with the actual model."),
        ("模型配置已回滚：小键盘的新建会话按键未被驱动接受。", "The model configuration was rolled back because the keypad's new-task key was not accepted by the driver."),
        ("模型配置已回滚：小键盘旋钮的预热清理指令未被驱动接受。", "The model configuration was rolled back because the keypad driver did not accept the prewarm-clear dial command."),
        ("Codex 未确认旧预热会话已清理，模型配置已回滚；请保持空白新会话在前台后重试。", "Codex did not confirm that the old prewarmed task was cleared, so the model configuration was rolled back. Keep the blank new task in the foreground and try again."),
        ("模型配置已回滚：小键盘旋钮按键未能打开推理选择器。", "The model configuration was rolled back because the keypad dial press could not open the reasoning selector."),
        ("Codex 未确认相邻推理档位，已回滚模型；请保持空白新会话在前台后重试。", "Codex did not confirm the adjacent reasoning level, so the model was rolled back. Keep the blank new task in the foreground and try again."),
        ("模型配置已回滚：小键盘旋钮未能恢复目标推理档位。", "The model configuration was rolled back because the keypad dial could not restore the target reasoning level."),
        ("Codex 未确认目标推理档位，已回滚模型；本次切换未生效。", "Codex did not confirm the target reasoning level, so the model was rolled back and this switch did not take effect."),
        ("模型配置已回滚：空白新会话的 NEW 重建指令未被驱动接受，或 ACT06 当前并非 NEW。", "The model configuration was rolled back because the blank task's NEW rebuild command was not accepted by the driver, or ACT06 is not currently mapped to NEW."),
        ("目标配置已保留，但 Codex 未提供新会话重建结果；请以输入框下方的模型显示为准，重新打开空白新会话后再试。", "The target config was kept, but Codex did not report the new-task rebuild outcome. Use the model shown below the composer as authoritative, then open a blank new task and try again."),
        ("Codex 已确认旋钮改变了前台新会话，但最终目标未确认；已保留最后可观察配置，请以输入框下方的模型显示为准。", "Codex confirmed that the dial changed the foreground new task, but the final target was not confirmed. The last observable config was kept; use the model shown below the composer as authoritative."),
        ("未能恢复小键盘旋钮模式，模型配置已回滚。", "The keypad dial mode could not be restored, so the model configuration was rolled back."),
        ("默认强度", "default effort"),
        ...
        ("快捷模型切换已取消；请以当前任务显示的模型为准。", "The quick-model switch was cancelled; use the model currently shown for this task."),
        ...
        ("快捷模型切换未完成", "Quick-model switch not completed"),
        ...
        ("额度重置：可用 {resets.Count} 次", "Usage limit resets: {resets.Count} available"),
        ("额度重置：暂不可用", "Usage limit resets: unavailable"),
        ("全额重置", "Full reset"),
        ("○ {title} · {expiration} 到期", "○ {title} · expires {expiration}"),
        ("周额度", "weekly limit"),
        ("天额度", "day limit"),
        ("小时额度", "hour limit"),
        ("分钟额度", "minute limit"),
        ...
        ("Agent 槽位 {slotId + 1} 已标记为未读；左击该键后清除。", "Agent slot {slotId + 1} was marked unread; left-click the key to clear it."),
        ("Agent 槽位 {slotId + 1} · AG{slotId:00} · {state} · 单击切换", "Agent slot {slotId + 1} · AG{slotId:00} · {state} · click to switch"),
        ("Agent 槽位", "Agent slot"),
