...
/// <summary>
/// Keeps only a small net encoder intent so a busy renderer never receives a
/// delayed replay of every historical wheel packet. Owned by the UI thread.
/// </summary>
internal sealed class EncoderStepAccumulator
{
        ...
        long maximumAgeTicks)
    {
        ...
        if (currentTimestamp >= _latestInputTimestamp &&
