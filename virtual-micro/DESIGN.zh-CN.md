...
`v.oai.thstatus` 都按同一完整快照应用；native service 因 inactivity 发出每个
熄灯周期的第一帧全槽 `off` 时，屏幕模拟器通过 VHF 回送一次 `ACT11` release。
该事件会恢复 Codex 保存的真实灯光模型，同时被当前 Micro bridge 明确忽略，
不会触发业务动作。后续全灭帧不会重复唤醒，直到收到新的非全灭快照。启动或重连
时没有真实快照就保持熄灭，不创建中性蓝色等屏幕专用灯色。
