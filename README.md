...
framework-dependent single-file zip is about 6.4 MiB and requires the .NET 10
Desktop Runtime x64. The `MICRO` title-bar and tray commands only launch the
...
The v1.1 Windows package is self-contained and does not require a separate .NET runtime. Agent Controller still launches without the driver, but its limited `NotSent` fallbacks are not full Micro compatibility.
...
For implementation status and edge cases, see the [v1 control reference](public/docs/controller-operations.md), [architecture and input flow](public/docs/architecture-and-input-flow.md), [Micro command reference](public/docs/codex-micro-command-reference.md), and [v1.1 release notes](public/docs/release-v1.1.md).
