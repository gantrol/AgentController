...
- `CodexMicroSimulator-v1.0.1-win-x64-portable`：自包含便携模拟器，无需另装 .NET Runtime。
...
```powershell
cd "$env:USERPROFILE\Downloads\CodexMicroSimulator-v1.0.1-win-x64-portable"
...
v1.0.1 对屏幕模拟器做了显示层特化：协议层仍正常应答 `off`，但界面保留最近一次有效灯色；如果程序启动时 Codex 已经闲置、还没有历史灯色，则使用中性蓝色常亮。下一帧真实任务状态到来后再继续实时更新。悬停第二盏状态灯时会显示“Codex 空闲照明已暂停 · 屏幕保留最近灯色 · 连接正常”。这不会伪造 HID 连接，也不会改变 Codex 的实体设备协议。
...
v1.0.1 改为三态策略：精确审核通过、未知版本黄灯继续、已知异常红灯阻止。它不会关闭已知版本的哈希校验。
...
- 六个 Agent 槽位在 Codex 进入 inactivity 状态后继续保留最近灯色；
