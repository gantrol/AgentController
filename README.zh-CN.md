...
- **右摇杆就是 Codex Micro 左上角旋钮**：推动摇杆会产生顺/逆时针档位，Codex 的 `composer-navigation` 负责遍历输入框工具、Model、Effort、Speed 和已打开的菜单；短按 **R3** 等同按下旋钮，长按则打开 Agent Controller 设置。Agent Controller 不再自行猜测当前模型菜单。
- 如何录音？一直按住 **LT**，松开就是停止。
- 如何发送？按 **X**。
...
这个 [Codex Micro 桌面模拟器](virtual-micro/README.zh-CN.md) 显示可缩放的透明小键盘，并且不依赖 Agent Controller。[v0.1.0 预发布版](../../releases/tag/codex-micro-v0.1.0) 提供无需另外安装 .NET Runtime 的 Windows x64 便携应用，以及单独标明的**未签名开发者驱动包**。该驱动包可以省掉 C/C++ 编译步骤，但不是正式安装包；请先[在本机二次签名并安装](virtual-micro/UNSIGNED-DRIVER.zh-CN.md)，或从源码自行构建驱动。
...
| 右摇杆 | 映射为 Micro 旋钮的顺/逆时针档位；由 Codex 当前的 encoder mode 和官方 bridge 解释，不再维护独立的简易/高级摇杆状态机。 |
