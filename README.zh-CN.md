...
这个 [Codex Micro 桌面模拟器](virtual-micro/README.zh-CN.md) 显示可缩放的透明小键盘，并且不依赖 Agent Controller。[v0.1.0 预发布版](../../releases/tag/codex-micro-v0.1.0) 已提供无需另外安装 .NET Runtime 的 Windows x64 便携应用。预编译驱动二进制仍不随包提供，请按照 `virtual-micro` 目录下的说明在本机编译并安装驱动。
