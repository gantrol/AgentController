...
- **右摇杆扩展 Codex Micro 左上角旋钮**：上/下通过 `ENC_CW / ENC_CC` 遍历 Advanced、Fast、Power 横条等控件；左/右保持屏幕方向，调整或进入当前控件。短按 **R3** 等同按下旋钮，长按则打开 Agent Controller 设置。两个轴不会再合并成同一档位流。
- 如何录音？一直按住 **LT**，松开就是停止。
- 如何发送？按 **X**。
...
| 右摇杆 | 上/下通过 Micro 旋钮档位选择控件；左/右按屏幕方向调整当前控件。仍由 Codex 的 encoder mode 和官方 bridge 持有遍历，不恢复独立的简易/高级状态机。 |
