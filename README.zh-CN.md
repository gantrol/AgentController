...
### Codex Micro 桌面模拟器：另一条实现路线

除了手柄路线，仓库现在也通过另一种方式实现并改良 Codex Micro：一套独立的桌面虚拟小键盘。目前阶段性成果是虚拟键盘已经与 Codex 桌面端联调成功，按键、白色旋钮、摇杆、Micro 设置路由和 Agent 状态报告均已走通 VHF/UMDF2 路径。

![Codex Micro 桌面模拟器](public/images/codex-micro-simulator.png)

这个 [Codex Micro 桌面模拟器](virtual-micro/README.zh-CN.md) 显示可缩放的透明小键盘，并且不依赖 Agent Controller。[v1.0.0 正式版](../../releases/tag/codex-micro-v1.0.0) 提供无需另外安装 .NET Runtime 的 Windows x64 便携应用，以及单独标明的**未签名开发者驱动包**。该驱动包可以省掉 C/C++ 编译步骤，但不是正式安装包；请先[在本机二次签名并安装](virtual-micro/UNSIGNED-DRIVER.zh-CN.md)，或从源码自行构建驱动。
...
6. 如需完整的 Micro-first HID 路径，请另外安装仓库中唯一受支持的 `CodexMicroVhfUm`（UMDF2/VHF）Device Support。当前只提供未签名开发者流程，请先阅读[安装教程](docs/CodexMicroSimulator-安装教程.zh-CN.md)和[未签名驱动说明](virtual-micro/UNSIGNED-DRIVER.zh-CN.md)。

v1.0.2 Windows 包为自包含版本，不需要另行安装 .NET Runtime。没有驱动时，Agent Controller 仍可运行，但只会对明确 `NotSent` 的操作尝试有限回退，不能视为完整 Micro 兼容模式。
...
更完整的实现状态和边界情况，请参阅 [v1 操作清单](public/docs/controller-operations.md)、[架构与输入链路](public/docs/architecture-and-input-flow.md)、[Micro 指令表](public/docs/codex-micro-command-reference.md)与 [v1.0.2 版本说明](public/docs/release-v1.0.2.md)。
...
- v1 尚未提供 macOS 版本、商业签名驱动安装器、可配置的 Agent/Command 槽位和完整 Plan 模式手柄路由。
...
- `virtual-micro/` —— Micro 模拟器、单 Broker、`CodexMicroVhfUm` 设备支持和兼容性测试；
