...
仓库还包含一个与 Agent Controller 完全独立、仅提供源码的 [Codex Micro 桌面模拟器](virtual-micro/README.zh-CN.md)。它显示可缩放的透明小键盘，并通过微软 VHF/UMDF2 虚拟 HID 直接连接 Codex。仓库不提供预编译驱动二进制，请按照 `virtual-micro` 目录下的说明在本机编译并安装驱动。
